...
class Expense_month(View):
    def get(self,request,pk):
        expense = Expense.objects.get(id=int(pk))
        category = expense.category
        date = str(expense.created)
        expense_month = Expense.objects.filter(category = category,created__year=date[:4],created__month =date[5:7])
        summa = 0
        for i in expense_month:
            summa += i.summa
        return JsonResponse({"status":"ok","summa":summa,'date':date[0:7],'category':category.title})
